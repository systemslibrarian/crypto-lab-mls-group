 ...
 *    files the node under `incomplete`. The hover states here are
 *    `color-mix(...)` tints over the elevated surface, so their text composites
 *    onto something axe will not always measure.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Note this lab reduces its
 *    animations to `0.01ms` rather than cancelling them, which is the safe
 *    pattern: nothing strands at `opacity: 0` under the preference.
 *
 ...
 *    the ratchet tree widens with every member added and the transcript prints
 *    long hex, so at 380px much of both is clipped away. Content scrolled past the client box is not dimmed or partly
